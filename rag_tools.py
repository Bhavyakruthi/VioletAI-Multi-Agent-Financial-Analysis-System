...
class SearchDocumentsTool(BaseTool):
    ...
    Use this tool to find specific information, facts, or passages related to a query.
    Returns relevant text chunks with their source documents.
    """
    ...
    def _run(self, query: str, num_results: int = 5) -> str:
        ...
        if self.document_processor is None:
            return "Error: Document processor not initialized. Please upload documents first."
...
class GetDocumentSummaryInput(BaseModel):
    ...
    document_name: Optional[str] = Field(
        default=None, 
        description="Name of specific document to summarize (optional)"
    )


class GetDocumentSummaryTool(BaseTool):
    """Tool for getting document content for summarization."""
    
    name: str = "get_document_content"
    description: str = """
    Get the full content of uploaded documents for summarization.
    Use this tool when you need to create a comprehensive summary of documents.
    """
...
def create_rag_tools(document_processor) -> List[BaseTool]:
    ...
    summary_tool.document_processor = document_processor
    
    list_tool = ListDocumentsTool()
    list_tool.document_processor = document_processor
    
    return [search_tool, summary_tool, list_tool]
